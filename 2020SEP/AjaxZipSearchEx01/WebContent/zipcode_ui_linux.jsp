...
// 						var json = eval(data);
			...
			// 상대경로 / 데이터와 ui가 같은 서버에 존재
// 			request.open( 'get', './data/zipcode_json.jsp?dong=' + encodeURIComponent(dong) , true);
			// 절대경로
			request.open( 'get', 'http://localhost:8080/AjaxZipSearchEx01/data/zipcode_json.jsp?dong=' + encodeURIComponent(dong) , true);
			
			// 원격(Linux)에 있는 데이터 요청
			// 분명히 http://192.168.45.129:9090/ajax01/data/zipcode_json.jsp?dong=%EA%B3%A0%EC%9E%94
			// 이렇게 검색하면 결과가 나오는데 ajax로 부르니 에러가 뜬다.
			// ajax는 같은 서버에 있는 데이터여야 가져올 수 있기 때문이다. 이를 크로스 도메인 문제라 한다.
// 			request.open( 'get', 'http://192.168.45.129:9090/ajax01/data/zipcode_json.jsp?dong=' + encodeURIComponent(dong) , true);
			request.open( 'get', 'http://localhost:8080/AjaxZipSearchEx01/zipcode_proxy.jsp?dong=' + encodeURIComponent(dong) , true);
			request.send();
		};
	};
</script>
