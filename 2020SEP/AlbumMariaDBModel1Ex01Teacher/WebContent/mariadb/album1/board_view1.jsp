...
<%
	...
	CommentTO cpto = new CommentTO();
	cpto.setPseq(request.getParameter( "seq" ));

	CommentDAO cdao = new CommentDAO();
	ArrayList<CommentTO> commentLists = cdao.commentList( cpto );
...
<script type="text/javascript">
	window.onload = function() {
		...
		var writeServer = function(writer, password, content) {
			...
			var url = './data/comment_write1_ok_json.jsp?pseq=<%= seq %>'; 
...
// 							sbHtml.append( "</tr>" );
			...
			request.open( 'get', './data/comment_list1_json.jsp?pseq=<%= seq %>', true);
			request.send();
		};
...
<%-- 			<%= sbHtml %> --%>
			...
			<form method="post" name="commentfrm">
			<table>
			<tr>
