...
<script type="text/javascript">
	window.onload = function() {
		document.getElementById('commentsubmit').onclick = function() {
			...
			console.log(document.getElementById('commentwriter').value.trim());
			
			if(document.getElementById('commentwriter').value.trim() == "") {
				alert('이름을 입력하셔야 합니다.');
				return false;				
			}
			if(document.getElementById('commentpassword').value.trim() == "") {
				...
				if (request.readyState == 4) {
					if (request.status == 200) {
						...
						for (var i=0; i<json.length; i++) {
							result += "<tr><td class='coment_re'><strong>"+ json[i][cwriter] +"</strong> (";
							result += json[i][cwdate] + ")<div class='coment_re_txt'>";
							result += "	</div></td></tr>";
						}
			...
			request.open( 'get', './comment_write1_ok.jsp', true);
			request.send();
		};
	};
</script>
...
<%-- 			<input type="hidden" name="pseq" value="<%=seq %>" />			 --%>
			...
			<tr>
				<td width="94%" class="coment_re">
					글쓴이 <input type="text" id="commentwriter" maxlength="5" class="coment_input" />&nbsp;&nbsp;
					비밀번호 <input type="password" id="commentpassword" class="coment_input pR10" />&nbsp;&nbsp;
				</td>
				<td width="6%" class="bg01"></td>
			</tr>
			<tr>
				<td class="bg01">
					<textarea id="commentcontent" cols="" rows="" class="coment_input_text"></textarea>
				</td>
				<td align="right" class="bg01">
					<button id="commentsubmit" class="btn_comment_write btn_txt01">댓글등록</button>
				</td>
			</tr>
			</table>
